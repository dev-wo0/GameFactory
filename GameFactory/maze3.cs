...
namespace GameFactory
{
    public partial class maze3 : Form
    {
        ...
        void findmaze(int i, int j)
        {
            ...
            if (j + 1 < 11 && form1.mazeB[i, j + 1] != '1' && form1.mazeB[i, j + 1] != '2' && find[i, j + 1] != '.')
            {
                findmaze(i, j + 1);
            }
            if (j - 1 >= 0 && form1.mazeB[i, j - 1] != '1' && form1.mazeB[i, j - 1] != '2' && find[i, j - 1] != '.')
            {
                findmaze(i, j - 1);
            }
            if (i + 1 < 11 && form1.mazeB[i + 1, j] != '1' && form1.mazeB[i + 1, j] != '2' && find[i + 1, j] != '.')
            {
                findmaze(i + 1, j);
            }
            if (i - 1 >= 0 && form1.mazeB[i - 1, j] != '1' && form1.mazeB[i - 1, j] != '2' && find[i - 1, j] != '.')
            {
                findmaze(i - 1, j);
            }
        ...
        private void Form3_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                if (form1.mazeB[a2y, a2x] != '0')
                {
                    MessageBox.Show("입구와 같은 위치에는 출구를 생성할 수 없습니다. \n다른곳에 출구를 생성해주세요.");
                    return;
                }

                form1.mazeB[a2y, a2x] = 'e';
            ...
            else if (count > 1 && count < 17)
            {
                ...
                if (direction == true) 
                {
                    ...
                    if (form1.mazeB[c2y, c2x] == '1')          //벽이 생성된 곳을 maze배열에 1을 대입
                    {
